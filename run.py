...
parser = argparse.ArgumentParser(description='MetaTCN')
...
parser.add_argument('--model', type=str, default='MetaTCN',
                    help='model name, options: [MetaTCN]')
...
parser.add_argument('--type_model', type=int, default=2, help='the type model')
parser.add_argument('--num_extract_blocks', type=int, default=3, help='the number of extract blocks')
parser.add_argument('--num_memory', type=int, default=2, help='the number of memory')
parser.add_argument('--act', type=str, default='identity', help='the number of memory')
...
if __name__ == '__main__':
    ...
    if args.is_training:
        for ii in range(args.itr):
            ...
            setting = '{}_{}_{}_ft{}_sl{}_ll{}_pl{}_dm{}_nh{}_el{}_dl{}_df{}_fc{}_eb{}_dt{}_{}_tm{}_nsb{}_nm{}_{}'.format(
                args.model_id,
                args.model,
                args.data,
                args.features,
        ...
        pre_setting = '{}_{}_{}_ft{}_sl{}_ll{}_pl{}'.format(
            args.model_id,
            args.model,
            args.data,
            args.features,
        ...
        exp = Exp(args)  # set experiments
        print('>>>>>>>testing : {}<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<'.format(setting))
        exp.test(setting, test=1, pre=pre_setting)
        torch.cuda.empty_cache()
