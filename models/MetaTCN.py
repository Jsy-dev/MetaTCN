...
from models.Layer import series_decomp, Flatten_Head
from models.MetaUnit import mateMeomery
...
class MetaTCN(nn.Module):
    def __init__(self,patch_size,patch_stride, stem_ratio, downsample_ratio, ffn_ratio, num_blocks, large_size, small_size, dims, dw_dims,
                 ...
                 subtract_last=False, freq=None, seq_len=512, c_in=7, individual=False, target_window=96, mum_memory=2, num_extract=3, type_model=1,act='identity'):
        ...
        if self.patch_size != self.patch_stride:
            self.output_len = [seq_len+(self.patch_size-self.patch_stride)]
        else:
            self.output_len = [seq_len]
        self.seq_len = seq_len
        self.output_len.append(math.floor((self.output_len[-1] + 2 * 0 - 1 * (patch_size - 1) - 1) / patch_stride + 1))
        self.c_in = c_in
        # RevIN
        self.revin = revin
        if self.revin:
        ...
        for i in range(3):
            if self.output_len[-1] % downsample_ratio != 0:
                current_L = math.floor((self.output_len[-1] + 2 * 0 - 1 * (downsample_ratio - 1) - 1) / downsample_ratio + 2)
            else:
                current_L = math.floor((self.output_len[-1] + 2 * 0 - 1 * (downsample_ratio - 1) - 1) / downsample_ratio + 1)
            self.output_len.append(current_L)


        self.back_len = self.output_len[::-1]
        self.back_len[-1] = self.target_window
        print('self.output_len',self.output_len)
        print('self.back_len',self.back_len)
        ...
        self.metaUnits = nn.ModuleList()
        for stage_idx in range(self.num_stage):
            if self.type_model!=0:
                MateLeaner = mateMeomery(mem_num=mum_memory, mem_dim=self.output_len[stage_idx+1], in_len=self.output_len[stage_idx+1], #+self.output_len[stage_idx]
                                    out_len=self.output_len[stage_idx+1], input_dim=dims[0], out_dim=dims[0], input_x_dim=self.c_in, input_x_len=self.seq_len, num_extract=num_extract, act=self.act)  # 16
                self.metaUnits.append(MateLeaner)
...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        self.num_extract_blocks = configs.num_extract_blocks
        self.num_memory = configs.num_memory
        self.type_model = configs.type_model
        ...
        if self.decomposition:
            ...
            self.model_res = MetaTCN(patch_size=self.patch_size,patch_stride=self.patch_stride,stem_ratio=self.stem_ratio, downsample_ratio=self.downsample_ratio, ffn_ratio=self.ffn_ratio, num_blocks=self.num_blocks, large_size=self.large_size, small_size=self.small_size, dims=self.dims, dw_dims=self.dw_dims,
                 ...
                 subtract_last=self.subtract_last, freq=self.freq, seq_len=self.seq_len, c_in=self.c_in, individual=self.individual, target_window=self.target_window,num_extract=self.num_extract_blocks,mum_memory=self.num_memory,type_model=self.type_model,act= self.act)
            self.model_trend = MetaTCN(patch_size=self.patch_size,patch_stride=self.patch_stride,stem_ratio=self.stem_ratio, downsample_ratio=self.downsample_ratio, ffn_ratio=self.ffn_ratio, num_blocks=self.num_blocks, large_size=self.large_size, small_size=self.small_size, dims=self.dims, dw_dims=self.dw_dims,
                 nvars=self.nvars, small_kernel_merged=self.small_kernel_merged, backbone_dropout=self.drop_backbone, head_dropout=self.drop_head, use_multi_scale=self.use_multi_scale, revin=self.revin, affine=self.affine,
                 subtract_last=self.subtract_last, freq=self.freq, seq_len=self.seq_len, c_in=self.c_in, individual=self.individual, target_window=self.target_window,num_extract=self.num_extract_blocks,mum_memory=self.num_memory,type_model=self.type_model,act= self.act)
        else:
            self.model = MetaTCN(patch_size=self.patch_size,patch_stride=self.patch_stride,stem_ratio=self.stem_ratio, downsample_ratio=self.downsample_ratio, ffn_ratio=self.ffn_ratio, num_blocks=self.num_blocks, large_size=self.large_size, small_size=self.small_size, dims=self.dims, dw_dims=self.dw_dims,
                 nvars=self.nvars, small_kernel_merged=self.small_kernel_merged, backbone_dropout=self.drop_backbone, head_dropout=self.drop_head, use_multi_scale=self.use_multi_scale, revin=self.revin, affine=self.affine,
                 subtract_last=self.subtract_last, freq=self.freq, seq_len=self.seq_len, c_in=self.c_in, individual=self.individual, target_window=self.target_window,num_extract=self.num_extract_blocks,mum_memory=self.num_memory,type_model=self.type_model,act= self.act)
